...
const FundingContextProvider = ({ children, }) => {
    ...
    const updateSupportsFunding = React.useCallback((...args_1) => _tslib.__awaiter(void 0, [...args_1], void 0, function* ({ address, token } = {}) {
        var _c;
        if (!fundingEnabled)
            return;
        if (!primaryWallet)
        ...
        try {
            const [{ url }] = yield onramp.getOnrampProviders({
                chain: chainName,
                environmentId,
