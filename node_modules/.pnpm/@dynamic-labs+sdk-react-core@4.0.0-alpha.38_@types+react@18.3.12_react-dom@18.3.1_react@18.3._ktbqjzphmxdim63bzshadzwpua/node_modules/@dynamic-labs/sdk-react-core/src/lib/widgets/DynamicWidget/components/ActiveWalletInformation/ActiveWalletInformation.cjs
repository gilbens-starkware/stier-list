...
const ActiveWalletInformation = ({ isLoading = false, }) => {
    ...
    const primaryWalletNativeBalance = () => primaryWallet ? (jsxRuntime.jsx(Balance.Balance, { className: 'balance-header__balance', wallet: primaryWallet, network: network })) : (jsxRuntime.jsx(Skeleton.Skeleton, { className: 'balance-header__skeleton' }));
    const handleScroll = () => {
        if (contentRef.current) {
            setHasShadow(contentRef.current.scrollTop > 0);
        }
    };
    const multiAssetHeader = () => (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [jsxRuntime.jsx("div", { className: `${hasShadow ? 'shadow' : ''}`, children: jsxRuntime.jsxs("button", { onClick: () => {
