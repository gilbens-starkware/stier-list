...
const Header = ({ onClose: handleClose, heading, projectSettings, copykey, }) => {
    ...
    const { goToInitialView, view, setView } = ViewContext.useViewContext();
    const { showBackButton, showCloseButton, showHelpContent, isWalletListTypeView, displayBorderBelowHeader, } = useAuthLayoutChecks.useAuthLayoutChecks();
    ...
    const handleBackClick = React.useCallback(() => {
        ...
        setError(undefined);
        if (view.type === 'no-qr-not-installed' &&
            (selectedWalletConnector === null || selectedWalletConnector === void 0 ? void 0 : selectedWalletConnector.canConnectViaQrCode)) {
            setView('qr-code');
            return;
        }
        goToInitialView();
    ...
    return (jsxRuntime.jsx(ModalHeader.ModalHeader, { leading: leadingButtons, trailing: closeButton || infoButton, displayBorder: displayBorderBelowHeader, children: (!projectSettings || Boolean(heading)) && (jsxRuntime.jsx(Typography.Typography, { as: 'h1', variant: 'title', color: 'primary', "data-testid": 'dynamic-auth-modal-heading', className: 'header__typography', copykey: copykey, children: !projectSettings ? (jsxRuntime.jsx(Skeleton.Skeleton, { className: 'header__skeleton' })) : (heading) })) }));
};
