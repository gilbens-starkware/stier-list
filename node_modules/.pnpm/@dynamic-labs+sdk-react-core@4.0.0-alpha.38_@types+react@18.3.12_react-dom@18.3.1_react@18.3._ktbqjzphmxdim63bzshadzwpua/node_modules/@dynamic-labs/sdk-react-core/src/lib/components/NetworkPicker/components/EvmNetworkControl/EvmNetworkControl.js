...
const EvmNetworkControl = ({ evmNetworks, className = '', activeClassName = '', network, walletConnector, isOpen, setIsOpen, showNetworkName = false, }) => {
    ...
    if (!network) {
        // If this connector is not available, just show a generic "Ethereum" network
        // As it will never be able to load a network
        if (walletConnector && !walletConnector.isAvailable) {
            return (jsxs("div", { "data-testid": 'EvmNetworkControl', className: classNames(className, {
                ...
                }), children: [jsx(Icon, { size: 'small', children: jsx("img", { src: fallbackChainData.iconUrl, alt: `${fallbackChainData.name} icon` }) }), showNetworkName && (jsx(Typography, { className: 'evm-network-control__network-name', as: 'span', variant: 'body_small', children: fallbackChainData.name }))] }));
        }
    ...
    if ((evmNetworks === null || evmNetworks === void 0 ? void 0 : evmNetworks.length) === 1) {
        ...
        return (jsxs("div", { "data-testid": 'EvmNetworkControl', onClick: () => network && setIsOpen(!isOpen), className: classNames('evm-network-control__container', {
                [activeClassName]: isOpen,
            }, className), children: [currentEvmNetwork.iconUrls.length > 0 && (jsx(Icon, { size: 'small', children: jsx("img", { src: currentEvmNetwork.iconUrls[0], alt: `${currentEvmNetwork.name} icon` }) })), showNetworkName && (jsx(Typography, { className: 'evm-network-control__network-name', as: 'span', variant: 'body_small', children: currentEvmNetwork.vanityName || currentEvmNetwork.name }))] }));
    }
