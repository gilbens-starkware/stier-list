...
var J = (t, e, r) => e in t ? Z(t, e, { enumerable: !0, configurable: !0, writable: !0, value: r }) : t[e] = r;
var B = (t, e, r) => (J(t, typeof e != "symbol" ? e + "" : e, r), r);
import { W as WebWalletConnector, D as DEFAULT_WEBWALLET_URL } from "./index-3eafdb9e.js";
import { A as ArgentMobileBaseConnector } from "./index-fd75fb40.js";
import { I as InjectedConnector, A as ARGENT_X_ICON } from "./index-d6560940.js";
...
var K = Object.defineProperty, X = (t, e, r) => e in t ? K(t, e, { enumerable: !0, configurable: !0, writable: !0, value: r }) : t[e] = r, Q = (t, e, r) => (X(t, typeof e != "symbol" ? e + "" : e, r), r), W = (t, e, r) => {
  if (!e.has(t))
    throw TypeError("Cannot " + r);
}, w = (t, e, r) => (W(t, e, "read from private field"), r ? r.call(t) : e.get(t)), z = (t, e, r) => {
  if (e.has(t))
    throw TypeError("Cannot add the same private member more than once");
  e instanceof WeakSet ? e.add(t) : e.set(t, r);
}, P = (t, e, r, n) => (W(t, e, "write to private field"), n ? n.call(t, r) : e.set(t, r), r), U = (t, e, r) => (W(t, e, "access private method"), r);
...
function satisfy(t, e) {
  ...
  for (const c of r) {
    ...
    const [, m, , h, p, y, b] = d, g = {
      operator: m,
      version: combineVersion(h, p, y, b),
      major: h,
      minor: p,
      patch: y,
      preRelease: b?.split(".")
      ...
      const { requiredVersion: l } = o, c = getFindShareFunction(s)(t, u, e), d = () => {
        if (o.singleton) {
          if (typeof l == "string" && !satisfy(c, l)) {
            const h = `Version ${c} from ${c && t[u][e][c].from} of shared singleton module ${e} does not satisfy the requirement of ${r.from} which needs ${l})`;
            o.strictVersion ? error(h) : warn$1(h);
          }
          return t[u][e][c];
        } else {
          if (l === !1 || l === "*" || satisfy(c, l))
            return t[u][e][c];
          for (const [h, p] of Object.entries(t[u][e]))
            if (satisfy(h, l))
              return p;
        }
      }, m = {
...
function generateSnapshotFromManifest(t) {
  ...
  if (!Object.keys(i).length) {
    var h;
    m = ((h = t.remotes) === null || h === void 0 ? void 0 : h.reduce(function(k, T) {
      var L, R = T.federationContainerName;
      return d.includes(R) ? L = a[R] : "version" in T ? L = T.version : L = T.entry, k[R] = {
        matchedVersion: L
      }, k;
    }, {})) || {};
  }
  Object.keys(i).forEach(function(k) {
    return m[k] = {
      matchedVersion: d.includes(k) ? a[k] : i[k]
    };
  });
  var p = t.metaData, y = p.remoteEntry, b = y.path, g = y.name, A = y.type, M = p.types, _ = p.buildInfo.buildVersion, I = p.globalName, E = t.exposes, S = {
    version: u || "",
    buildVersion: _,
    globalName: I,
    ...
    remotesInfo: m,
    shared: t?.shared.map(function(k) {
      return {
        assets: k.assets,
        sharedName: k.name
      };
    }),
    modules: E?.map(function(k) {
      return {
        moduleName: k.name,
        modulePath: k.path,
        assets: k.assets
  ...
  if (!((n = t.metaData) === null || n === void 0) && n.prefetchEntry) {
    var D = t.metaData.prefetchEntry, O = D.path, F = D.name, q = D.type;
    S = _object_spread_props(_object_spread$1({}, S), {
      prefetchEntry: simpleJoinRemoteEntry(O, F),
      prefetchEntryType: q
    });
  }
  return "publicPath" in t.metaData ? l = _object_spread_props(_object_spread$1({}, S), {
    publicPath: c()
  }) : l = _object_spread_props(_object_spread$1({}, S), {
    getPublicPath: c()
  }), l;
}
...
function generatePreloadAssets(t, e, r, n, o) {
  ...
  traverseModuleInfo(n, r, (p, y, b) => {
    let g;
    if (b)
      g = d;
    else if (Array.isArray(m)) {
      const S = m.find((N) => N.nameOrAlias === y.name || N.nameOrAlias === y.alias);
      if (!S)
        return;
      g = defaultPreloadArgs(S);
    } else if (m === !0)
      g = d;
    else
      return;
    const A = getResourceUrl(p, "remoteEntry" in p ? p.remoteEntry : "");
    A && a.push({
      name: y.name,
      moduleInfo: {
        name: y.name,
    ...
    const _ = normalizePreloadExposes(g.exposes);
    if (_.length && "modules" in p) {
      var I;
      M = p == null || (I = p.modules) == null ? void 0 : I.reduce((S, N) => (_?.indexOf(N.moduleName) !== -1 && S.push(N), S), []);
    }
    function E(S) {
      const N = S.map((D) => getResourceUrl(p, D));
      return g.filter ? N.filter(g.filter) : N;
    }
    if (M) {
      const S = M.length;
      for (let N = 0; N < S; N++) {
        const D = M[N], O = `${y.name}/${D.moduleName}`;
        t.hooks.lifecycle.handlePreloadModule.emit({
          id: D.moduleName === "." ? y.name : O,
          name: y.name,
          remoteSnapshot: p,
          preloadConfig: g,
          remote: y,
          origin: t
        }), !getPreloaded(O) && (g.resourceCategory === "all" ? (i.push(...E(D.assets.css.async)), i.push(...E(D.assets.css.sync)), s.push(...E(D.assets.js.async)), s.push(...E(D.assets.js.sync))) : (g.resourceCategory = "sync") && (i.push(...E(D.assets.css.sync)), s.push(...E(D.assets.js.sync))), setPreloaded(O));
      }
    }
  }, !0, {}, o), o.shared && o.shared.forEach((p) => {
    var y;
    const b = (y = c.shared) == null ? void 0 : y[p.sharedName];
    if (!b)
      return;
    const g = getRegisteredShare(t.shareScopeMap, p.sharedName, b, t.hooks.lifecycle.resolveShare);
    g && typeof g.lib == "function" && (p.assets.js.sync.forEach((A) => {
      u.add(A);
    }), p.assets.css.sync.forEach((A) => {
      l.add(A);
    }));
  });
  const h = s.filter((p) => !u.has(p));
  return {
...
class FederationHost {
  ...
  initializeSharing(e = DEFAULT_SCOPE, r) {
    ...
    const i = n[e], s = (c, d) => {
      var m;
      const { version: h, eager: p } = d;
      i[c] = i[c] || {};
      const y = i[c], b = y[h], g = !!(b && (b.eager || (m = b.shareConfig) != null && m.eager));
      (!b || b.strategy !== "loaded-first" && !b.loaded && (!p != !g ? p : o > b.from)) && (y[h] = d);
...
const metaMaskVirtualWallet = {
  ...
  async loadWallet(t) {
    ...
    const e = await loadRemote("MetaMaskStarknetSnapWallet/index"), { MetaMaskSnapWallet: r, MetaMaskSnap: n } = e, o = await n.GetProvider(t);
    return new r(o, "*");
  },
  async hasSupport(t) {
    return await detectMetamaskSupport(t);
  }
};
var Y, $;
const wallets = [
  ...
  },
  {
    ...
    downloads: {
      ...
      ios: `https://link.braavos.app/dapp/${(Y = ssrSafeWindow?.location) == null ? void 0 : Y.host}`,
...
class LocalStorageWrapper {
  constructor(e) {
    z(this, H), z(this, C), z(this, v, !1), z(this, j, void 0), z(this, x, void 0), Q(this, "value"), P(this, x, e), U(this, C, G).call(this);
  }
  set(e) {
    return !w(this, v) && !U(this, C, G).call(this) ? !1 : (this.delete(), this.value = e, e && (P(this, j, `${w(this, x)}-${generateUID()}`), localStorage.setItem(w(this, j), e)), !0);
  }
  get() {
    return U(this, H, V).call(this), this.value;
  }
  delete() {
    return !w(this, v) && !U(this, C, G).call(this) ? !1 : (this.value = null, w(this, j) && localStorage.removeItem(w(this, j)), !0);
  }
}
v = /* @__PURE__ */ new WeakMap(), j = /* @__PURE__ */ new WeakMap(), x = /* @__PURE__ */ new WeakMap(), H = /* @__PURE__ */ new WeakSet(), V = function() {
  this.value && this.set(this.value);
}, C = /* @__PURE__ */ new WeakSet(), G = function() {
  try {
    !w(this, v) && typeof window < "u" && (P(this, j, Object.keys(localStorage).find(
      (t) => t.startsWith(w(this, x))
    )), P(this, v, !0), w(this, j) && this.set(localStorage.getItem(w(this, j))));
  } catch (t) {
    console.warn(t);
  }
  return w(this, v);
...
const main = getStarknet(), defaultConnectors = ({
  ...
  )), n.push(new ArgentMobileBaseConnector(t)), n.push(new WebWalletConnector({ url: e })), n;
}, BROWSER_ALIASES_MAP = {
...
const mapModalWallets = ({
  ...
  return (o ? t : [
    ...
    const d = e.find((h) => h.id === c.id);
    if (d) {
      const h = d.id === "argentX" ? { light: ARGENT_X_ICON, dark: ARGENT_X_ICON } : isString(d.icon) ? { light: d.icon, dark: d.icon } : d.icon;
      return {
        name: d.name,
        id: d.id,
        icon: h,
    ...
    if (m) {
      const { downloads: h } = m, p = m.id === "argentX" ? ARGENT_X_ICON : m.icon;
      return {
        name: m.name,
        id: m.id,
        icon: { light: p, dark: p },
...
function create_else_block(t) {
  ...
  function y(A, M) {
    return (
      /*loadingItem*/
      A[2] === /*wallet*/
      A[0]?.id ? create_if_block_1 : (
        /*isSvg*/
        A[4] ? create_if_block_2 : create_else_block_1
      )
    );
  }
  let b = y(t), g = b(t);
  return {
    ...
    m(A, M) {
      insert(A, e, M), append(e, r), append(e, n), append(e, o), append(o, i), append(i, a), append(o, u), append(o, l), append(l, d), append(e, m), g.m(e, null), h || (p = [
        listen(
          e,
          "click",
          /*click_handler_1*/
    ...
    p(A, M) {
      ...
      1 && s !== (s = /*wallet*/
      (A[0].title ?? /*wallet*/
      A[0].name) + "") && set_data(a, s), M & /*wallet*/
      1 && c !== (c = /*wallet*/
      (A[0].subtitle ?? "") + "") && set_data(d, c), b === (b = y(A)) && g ? g.p(A, M) : (g.d(1), g = b(A), g && (g.c(), g.m(e, null)));
    },
    d(A) {
      A && detach(e), g.d(), h = !1, run_all(p);
    }
  };
}
  ...
  return {
    c() {
      ...
      t[3]) || attr(c, "src", m), attr(c, "class", "w-8 h-8 rounded-full"), attr(r, "class", `flex flex-row-reverse justify-between items-center 
              p-3 rounded-md cursor-pointer shadow-list-item 
              dark:shadow-none dark:bg-neutral-800 dark:text-white 
            hover:bg-neutral-100 dark:hover:bg-neutral-700`), attr(e, "aria-label", h = /*wallet*/
    ...
    m(g, A) {
      insert(g, e, A), append(e, r), append(r, n), append(r, o), append(r, i), append(i, s), append(i, u), append(r, l), append(r, c), y || (b = [
        listen(
          r,
          "click",
          /*click_handler*/
    ...
    p(g, A) {
      A & /*wallet*/
      1 && a !== (a = /*wallet*/
      g[0].name + "") && set_data(u, a), A & /*wallet*/
      1 && d !== (d = /*wallet*/
      g[0].name) && attr(c, "alt", d), A & /*wallet*/
      1 && h !== (h = /*wallet*/
      g[0].name + " download link") && attr(e, "aria-label", h), A & /*wallet*/
      1 && p !== (p = /*wallet*/
      g[0].download) && attr(e, "href", p);
    },
    d(g) {
      g && detach(e), y = !1, run_all(b);
    }
  };
}
...
function instance$1(t, e, r) {
  ...
  const a = typeof n.icon == "string" ? n.icon : o === "dark" ? n.icon.dark : n.icon.light, u = a?.startsWith("<svg"), l = () => {
    ...
    h.key === "Enter" && i(n.connector);
  };
  return t.$$set = (h) => {
    "wallet" in h && r(0, n = h.wallet), "theme" in h && r(5, o = h.theme), "cb" in h && r(1, i = h.cb), "loadingItem" in h && r(2, s = h.loadingItem);
  }, [
    n,
    i,
    s,
...
function create_if_block(t) {
  let e, r, n, o, i, s, a, u, l, c, d, m, h, p, y, b = ensure_array_like(
    /*modalWallets*/
    t[1]
  ), g = [];
  for (let M = 0; M < b.length; M += 1)
    g[M] = create_each_block(get_each_context(t, b, M));
  const A = (M) => transition_out(g[M], 1, 1, () => {
    g[M] = null;
  });
  return {
    c() {
    ...
    m(M, _) {
      insert(M, e, _), append(e, r), append(r, n), append(n, o), append(n, i), append(n, s), append(s, a), append(n, u), append(n, l), append(r, c), append(r, d);
    ...
    p(M, _) {
      ...
      1) && set_data(
        ...
        /*dappName*/
        M[0]
      ), _ & /*modalWallets, loadingItem, cb, theme*/
      78) {
        b = ensure_array_like(
          /*modalWallets*/
          M[1]
        ...
        for (I = 0; I < b.length; I += 1) {
          const E = get_each_context(M, b, I);
          g[I] ? (g[I].p(E, _), transition_in(g[I], 1)) : (g[I] = create_each_block(E), g[I].c(), transition_in(g[I], 1), g[I].m(d, null));
        }
        for (group_outros(), I = b.length; I < g.length; I += 1)
          A(I);
        check_outros();
      }
      (!h || _ & /*darkModeControlClass*/
    ...
    d(M) {
      M && detach(e), destroy_each(g, M), p = !1, run_all(y);
    }
  };
}
...
function instance(t, e, r) {
  ...
  onMount(async () => {
    if (s === "dark" || s === null && window.matchMedia("(prefers-color-scheme: dark)").matches ? r(4, m = "dark") : r(4, m = ""), l && window?.starknet_argentX) {
      try {
  ...
  return t.$$set = (g) => {
    "dappName" in g && r(0, n = g.dappName), "modalWallets" in g && r(1, o = g.modalWallets), "callback" in g && r(7, i = g.callback), "theme" in g && r(2, s = g.theme);
  }, [
    n,
    o,
    s,
...
const connect = async ({
  ...
  if (t === "canAsk" && c && ((await main.getAuthorizedWallets(i)).find((b) => b.id === c) ?? d.length === 1 ? d[0] : void 0)) {
    const b = l.find((A) => A.id === c);
    let g = null;
    return o === "wallet" && (g = await b?.connect() ?? null), b && (selectedConnector = b), {
  ...
  const m = mapModalWallets({
    ...
    b.id = p, document.body.appendChild(b);
    const g = b.attachShadow({ mode: "open" });
    return g.innerHTML = `<style>${css}</style>`, g;
  };
  return new Promise((p, y) => {
    const b = new Modal({
      ...
      props: {
        ...
        callback: async (g) => {
          try {
            if (selectedConnector = g, o === "wallet") {
              const A = await g?.connect() ?? null;
              g !== null && setStarknetLastConnectedWallet(g.id), p({
                connector: g,
                connectorData: A,
                wallet: g?.wallet ?? null
              });
            } else
              p({
                connector: g,
